<?php
    ...
    class PDF extends FPDF{   
        ...
        function Header(){                         
            ...
            $this->SetX(50);            
            $this->Cell(190, 5, utf8_decode("REPORTE POR TIPOS DE DOCUMENTOS " . maxCaracter(strtoupper($_GET['tipo']),20)),0,1, 'L',0);                                                                                                                            
            $this->SetFont('Amble-Regular','',10);        
            $this->Ln(3);
    ...
    if($_SESSION['tipo_user'] == '1'){
        $sql = "select nombres_usuario,nombre_archivo,fecha_cambios,asunto_cambio,nombre_medio,nombre_doc,archivo.estado from archivo,bitacora,usuario,tipo_documento,medio_recepcion where archivo.id_archivo = bitacora.id_archivo and bitacora.id_tipo_documento =tipo_documento.id_tipo_documento and bitacora.id_medio_recepcion = medio_recepcion.id_medio and bitacora.id_usuario = usuario.id_usuario and bitacora.id_tipo_documento = '".$_GET['id']."'";
    }else{
        $sql = "select nombres_usuario,nombre_archivo,fecha_cambios,asunto_cambio,nombre_medio,nombre_doc,archivo.estado from archivo,bitacora,usuario,tipo_documento,medio_recepcion where archivo.id_archivo = bitacora.id_archivo and bitacora.id_tipo_documento =tipo_documento.id_tipo_documento and bitacora.id_medio_recepcion = medio_recepcion.id_medio and bitacora.id_usuario = usuario.id_usuario and usuario.id_usuario = '".$_SESSION['id_gestion']."' and bitacora.id_tipo_documento = '".$_GET['id']."'";    
    }
    $sql = pg_query($sql);
    $repetido = 0;
